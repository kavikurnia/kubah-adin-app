// SHA-256 for deterministic idempotency identifiers, using UTF-8 input.
export function sha256(text){
 const bytes=new TextEncoder().encode(text),length=Math.ceil((bytes.length+9)/64)*64,data=new Uint8Array(length);data.set(bytes);data[bytes.length]=128;
 const view=new DataView(data.buffer);view.setUint32(length-8,Math.floor(bytes.length/536870912));view.setUint32(length-4,(bytes.length*8)>>>0);
