...
async function updateOrder(orderId, patch, historyLabel) {
  ...
  if (order?.schemaVersion === 2) {
    window.location.href = 'admin-website.html?v=product-validation-20260925-r23#orders/' + encodeURIComponent(orderId); return;
  }
...
function showOperational(module){
  ...
  if(['messages','shipping','couriers','slots','products','suppliers','webcontent','calculator','neworder','pricing'].includes(page)){frame.src='planning-admin.html?v=product-validation-20260925-r23&embed=1#'+(({couriers:'shipping/kurir',slots:'shipping/jadwal',pricing:'products/migrasi'})[module]||module);}
  else frame.src=page==='resellers'?'reseller-admin.html?v=product-validation-20260925-r23&embed=1#'+(module.split('/')[1]||'ringkasan'):'admin-website.html?v=product-validation-20260925-r23&embed=1#'+module;
...
function selectCalculatorTab(tab){
 ...
 const frame=document.getElementById('comparison-frame');if(current==='perbandingan'&&!frame.getAttribute('src'))frame.src='planning-admin.html?v=product-validation-20260925-r23&embed=1#calculator';
