...
async function api(action,data={}){return withDeadline(import('./manual-api.js?v=katalog-penawaran-20260909-r4').then(async m=>m.manualApi(await connect(),action,data)),30000,'manual/unavailable');}
...
async function updateOrder(orderId, patch, historyLabel) {
  ...
  if (order?.schemaVersion === 2) {
    window.location.href = 'admin-website.html?v=katalog-penawaran-20260909-r4#orders/' + encodeURIComponent(orderId); return;
  }
...
function showOperational(module){
  ...
  if(['shipping','couriers','slots','products','suppliers','webcontent','calculator','neworder','pricing'].includes(page)){frame.src='planning-admin.html?v=katalog-penawaran-20260909-r4&embed=1#'+(({couriers:'shipping/kurir',slots:'shipping/jadwal',pricing:'products/migrasi'})[module]||module);}
  else frame.src=page==='resellers'?'reseller-admin.html?v=katalog-penawaran-20260909-r4&embed=1#'+(module.split('/')[1]||'ringkasan'):'admin-website.html?v=katalog-penawaran-20260909-r4&embed=1#'+module;
...
function bindAdminDashboard(){
  ...
  document.getElementById('export-orders-channel').onclick=async()=>{try{const {csv,CHANNELS,salesChannel}=await import('./planning-domain.js?v=admin-supplier-20260909-r3');const {download}=await import('./planning-ui.js?v=katalog-penawaran-20260909-r4');const q=state.orderSearch.trim().toLowerCase(),rows=state.orders.filter(o=>(state.orderChannel==='semua'||orderChannel(o)===state.orderChannel)&&(state.orderTab==='semua'||o.status===state.orderTab)&&(!q||[o.invoiceNo,o.customerName].some(v=>String(v||'').toLowerCase().includes(q))));download('Pesanan-Sales-Channel.csv',csv(rows,[{key:'id',label:'ID pesanan'},{key:'invoiceNo',label:'No. Invoice'},{key:'customerName',label:'Pelanggan'},{key:'total',label:'Total'},{key:'salesChannel',label:'Sales Channel',value:o=>CHANNELS[salesChannel(o)]},{key:'status',label:'Status'}]));}catch(e){showToast(e.message);}};
...
function selectCalculatorTab(tab){
 ...
 const frame=document.getElementById('comparison-frame');if(current==='perbandingan'&&!frame.getAttribute('src'))frame.src='planning-admin.html?v=katalog-penawaran-20260909-r4&embed=1#calculator';
