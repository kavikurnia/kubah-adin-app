...
async function api(action,data={}){return withDeadline(import('./manual-api.js?v=checkout-cloud-20260909-r5').then(async m=>m.manualApi(await connect(),action,data)),30000,'manual/unavailable');}
...
function bindAdminDashboard(){
  ...
  document.getElementById('export-orders-channel').onclick=async()=>{try{const {csv,CHANNELS,salesChannel}=await import('./planning-domain.js?v=checkout-cloud-20260909-r5');const {download}=await import('./planning-ui.js?v=checkout-cloud-20260909-r5');const q=state.orderSearch.trim().toLowerCase(),rows=state.orders.filter(o=>(state.orderChannel==='semua'||orderChannel(o)===state.orderChannel)&&(state.orderTab==='semua'||o.status===state.orderTab)&&(!q||[o.invoiceNo,o.customerName].some(v=>String(v||'').toLowerCase().includes(q))));download('Pesanan-Sales-Channel.csv',csv(rows,[{key:'id',label:'ID pesanan'},{key:'invoiceNo',label:'No. Invoice'},{key:'customerName',label:'Pelanggan'},{key:'total',label:'Total'},{key:'salesChannel',label:'Sales Channel',value:o=>CHANNELS[salesChannel(o)]},{key:'status',label:'Status'}]));}catch(e){showToast(e.message);}};
