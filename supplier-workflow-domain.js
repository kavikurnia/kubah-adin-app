...
import {day,wibDay} from './planning-domain.js?v=katalog-pembeli-20260909-r7';
...
export const applicationNumber=uid=>'SUP-'+hash(uid).slice(0,16).toUpperCase();
export function applicationInput(d){const result={};for(const [k,max] of Object.entries({name:150,pic:150,phone:30,email:150,address:1500,category:200,productName:250,priceRange:300,minPurchase:200,capacity:200,offer:4000,notes:2000}))result[k]=str(String(d[k]||''),max,!['email','minPurchase','capacity','offer','notes'].includes(k));check(!result.email||/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(result.email),'Email tidak valid.');check(/^[+\d\s()-]{7,30}$/.test(result.phone),'Nomor kontak tidak valid.');return result;}
